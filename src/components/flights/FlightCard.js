    ...
    return (
      <div style={{ height: '97%', width: '97%', borderRadius: 8, backgroundColor: grey[200] }}>
        ...
        >
          ...
          <span onClick={()=>this.editFlight(details)}>EDIT</span> &nbsp;
          <span onClick={()=>this.delete(details)}>DELETE</span>
        </div>
        <div style={{cursor: 'pointer', position: 'relative', zIndex:99}} onClick={()=>this.addVote(details)}>
...
const mapDispatchToProps = (dispatch) => {
  return ({
    addVote(obj) {
      dispatch(uiAction.addVote(obj))
    },
    delete(obj) {
      dispatch(uiAction.delete(obj))
    },
  })
}
